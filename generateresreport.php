...
$pdf->Cell(45,8,'AUTHOR/SPONSOR',1, 0,'C');
...
while ($row = $result->fetch_assoc()) {
    ...
    $pdf->Rect($x + $w[0] + $w[1], $y, $w[2], $rowHeight); // AUTHOR/SPONSOR
    $pdf->Rect($x + $w[0] + $w[1] + $w[2], $y, $w[3], $rowHeight); // DATE APPROVED
    ...
    $pdf->MultiCell($w[2], $lineHeight, $author, 0, 'C');
    $pdf->SetXY($x + $w[0] + $w[1] + $w[2], $y);
    $pdf->MultiCell($w[3], $lineHeight, $date_approved, 0, 'C'); // wrap date
...
if ($rowCount === 0) {
    $pdf->Cell(273,8,'No records found for the selected date range.',1,1,'C');
}

<?php
    ...
    if (isset($_GET["id"]) && is_numeric($_GET["id"])) {
        ...
        // Fetch the ordinance title before deleting
        $fetch_sql = "SELECT title FROM ordinance WHERE id = ?";
        ...
        if (!empty($title)) {
            ...
            if ($stmt) {
                ...
                if (mysqli_stmt_affected_rows($stmt) > 0) {
                    ...
                    $log_sql = "INSERT INTO history_log (action, file_type, file_id, title) 
                                VALUES ('Deleted', 'Ordinance', ?, ?)";
                    $log_stmt = mysqli_prepare($conn, $log_sql);
                    mysqli_stmt_bind_param($log_stmt, "is", $id, $title);
                    mysqli_stmt_execute($log_stmt);
                    mysqli_stmt_close($log_stmt);

                    header("Location: files-ordinances.php?msg=Ordinance '$title' deleted successfully");
                    exit();
                } else {
            ...
            echo "Failed: Ordinance title not found.";
        }
    } else {
